...
+function ($) {
  "use strict";
    ...
    $.popup_dvb = function (modal,zindex,callback,maskcallBack) {
        //remove
        $(".modal-overlay").remove();
        var mask =  $("<div class='modal-overlay'/>");
        var z_index = zindex||11;
        ...
        modal.show();
        setTimeout(function(){
             modal.addClass("popupBox_show");
        },100);
        if(callback){
            callback();
        }
       mask.on("click",function(){
        modal.removeClass("popupBox_show");
        setTimeout(function(){
             modal.hide();
             mask.remove();
        },300);
    ...
    $.popup_dvb_close = function(modal,callback){
        $(".modal-overlay").remove();
        modal.removeClass("popupBox_show");
        setTimeout(function(){
             modal.hide();
    ...
    //显示一个消息，会在2秒钟后自动消失
    $.toast = function(msg, time, overlay) {
      var needoverlay = overlay||1;
      var $toast = $("<div class='modal toast'>"+msg+"</div>").appendTo(document.body);
      $.openModal($toast,needoverlay);
      setTimeout(function() {
        $.closeModal($toast,needoverlay);
      }, time || 2000);
    };
    $.openModal = function (modal,needoverlay) {
        ...
        var overlay;
        var isNeedOverlay = needoverlay||1;
        if(isNeedOverlay==1){
        ...
        if (!isLoginScreen && !isPickerModal && !isToast && isNeedOverlay==1) overlay.addClass('modal-overlay-visible');
    ...
    $.closeModal = function (modal,hasOverlay) {
