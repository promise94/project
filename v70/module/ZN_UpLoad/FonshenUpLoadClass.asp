...
Class UpLoadClass
	...
	Private m_dicForm,m_binForm,m_binItem,m_strDate,m_lngTime,m_NUP
	...
	Private Sub Class_Initialize
		...
		m_NUP = array("png","jpg","jpeg","gif","bmp","swf")
	End Sub
		...
		Do
			...
			if not m_dicForm.Exists(strInam&"_From") then
				...
				if m_binItem.Size<>0 then
					...
					Dim checknup:checknup = true
					for each ho in m_NUP
						if lcase(strFext)=lcase(ho) then
						checknup=false
						end if
					next
					if checknup then response.End()	
