...
Class FCKeditor
	...
	Public Property Let CreateID(CreateIDvalue)
		If ( IsNull( CreateIDvalue ) OR IsEmpty( CreateIDvalue ) ) Then
			sCreateID=""
		Else
			sCreateID=CreateIDvalue
		End If
	End Property
	...
	Public Function CreateHtml( instanceName )
		...
		If IsCompatible() Then
			...
			if isnull(sCreateID)=false and sCreateID<>"" then
				sLink = sBasePath & "editor/" & sFile & "?InstanceName=" + sCreateID
				...
				html = html & "<input type=""hidden"" id=""" & sCreateID & """ name=""" & instanceName & """ value=""" & Server.HTMLEncode( sValue ) & """ style=""display:none"" />"
	
				' Render the configurations hidden field.
				html = html & "<input type=""hidden"" id=""" & sCreateID & "___Config"" value=""" & GetConfigFieldString() & """ style=""display:none"" />"
	
				' Render the editor IFRAME.
				html = html & "<iframe id=""" & sCreateID & "___Frame"" src=""" & sLink & """ width=""" & sWidth & """ height=""" & sHeight & """ frameborder=""0"" scrolling=""no""></iframe>"
			else
